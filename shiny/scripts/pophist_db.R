...
config.data <- jsonlite::fromJSON(paste(readLines("../../env/config.json"), collapse=""))
...
getPopList <- function(){
    f.xls <- config.data$xls_pop_file
    pops <- read_excel(f.xls, "iberiat-pops") %>% mutate(acronym=tolower(acronym)) %>%
        rename(region=region_name_abrev)
    return(pops)
}
...
getDataStatus <- function(){
    ...
    orthos.dir <- paste0(config.data$data_dir, "/orthos")
...
plotArticializationDegree <- function(region.name){
    ...
    p <- ggplot(df.lc.summary) + geom_line(aes(x=year, y=ad, colour=acronym, group=acronym)) +
        facet_wrap(~ acr.name, ncol=4)
    p <- p + ylab("") + xlab("Human impact degree (0=least, 1=maximum)")
